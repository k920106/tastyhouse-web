...
방법 1: 페이지 전체를 한 번에 생성하도록 요청 (가장 간단한 명령)  
이 방법은 페이지 전체의 구현을 AI에게 맡기는 방식입니다.  
@src/app/login/page.tsx 파일을 다음 Zeplin 링크를 기반으로 업데이트해줘: https://zpl.io/257lQlJ  
요구사항:

- 링크에 있는 로그인 UI 전체를 구현해줘.
- UI는 Tailwind CSS를 사용해서 스타일링해줘.
- 프로젝트의 기존 코드 스타일과 규칙을 따라줘.

---

방법 2: 재사용 가능한 컴포넌트로 분리하여 요청 (권장하는 방법)

이 방법은 먼저 재사용 가능한 UI 컴포넌트를 만들고, 그 다음에 페이지에 조립하도록 요청하는 방식입니다. 이렇게 하면 코드가 더 체계화되고 유지보수가 쉬워집니다.

1단계: 로그인 폼 컴포넌트 생성 요청

새로운 파일을 @src/components/forms/LoginForm.tsx 경로에 생성해줘.
그리고 이 파일에 아래 Zeplin 링크의 '로그인 폼' 부분만 재사용 가능한 컴포넌트로 구현해줘: https://zpl.io/257lQlJ
요구사항:

- 이메일, 비밀번호 입력 필드와 로그인 버튼을 포함해줘.
